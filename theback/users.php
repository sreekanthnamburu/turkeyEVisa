...
<p class="alert alert-info">User Monthly Application count will Count from 3rd October 2015 and based on visa sent Date. And count will start from 1st of Every month. </p>
								<table id="sample-table-2" class="table table-striped table-bordered table-hover">
									...
									<?php //while ($applications) {
									 while($value=$result->fetch_assoc()){?>
                                                                                 <?php
											$query = "SELECT COUNT(*) as num FROM applications where lockUser='".$value['id']."'";
	$result1 = $mysqli->query($query);
	$total_pages = $result1->fetch_assoc();?>
	 <?php
											$query1 = "SELECT COUNT(*) as num FROM applications where MONTH(visasentdate)=MONTH(CURDATE()) and YEAR(visasentdate)=YEAR(CURDATE()) and lockUser='".$value['id']."'";
	$curmonth = $mysqli->query($query1);
	$cur_total_pages = $curmonth->fetch_assoc();?>
	 <?php
											$query2 = "SELECT COUNT(*) as num FROM applications where MONTH(visasentdate)=MONTH(CURDATE())-1 and YEAR(visasentdate)=YEAR(CURDATE()) and lockUser='".$value['id']."'";
	...
	$prev_total_pages = $prevmonth->fetch_assoc();?>
										...
										<!--
                                            ...
                                            <td><?php echo $total_pages['num'];?></td>
                                            <td><?php echo $cur_total_pages['num'];?></td>
                                            <td><?php echo $prev_total_pages['num'];?></td>
